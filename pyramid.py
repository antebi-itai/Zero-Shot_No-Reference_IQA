...
def create_image_pyramids(image, patch_size=5, min_dim_size=40):
    ...
    num_scales = floor(2 * log2(min(h, w) / min_dim_size)) + 1

    # desired highest level shape
    max_shrink_scale = (2 ** 0.5) ** (num_scales - 1)
    h = max_shrink_scale * (h // max_shrink_scale)
    w = max_shrink_scale * (w // max_shrink_scale)

    # full_size_image pyramid
    full_size_image_pyramid_shapes = [(int(h/(2**(0.5*i))), int(w/(2**(0.5*i))),) for i in range(num_scales)]
    ...
    pixels_outside_image_pyramid = [round(border_width * ((2 ** 0.5) ** (num_scales - 1 - i))) for i in range(num_scales)]
